...
func (s *Scanner) scanToken() {
	...
	default:
		if s.isDigit(c) {
			...
			// since we return errors, we never try to execute any of the code
		}
	}
}
...
func (s *Scanner) string() {
	...
	if s.isAtEnd() { // check if the loop ended because we reached the end of the file
		s.errors = append(s.errors, fmt.Errorf("Unterminated string"))
	}
...
func (s *Scanner) blockComment() {
	...
	if depth > 0 {
		s.errors = append(s.errors, fmt.Errorf("Unterminated block comment"))
	}
}
